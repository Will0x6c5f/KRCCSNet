...
def main():
    ...
    if args.model == 'krccsnet':
        model=krccsnet.build_krccsnet(sensing_rate=args.sensing_rate)


    criterion = loss_fn

    optimizer = optim.Adam(model.decoder.parameters(), lr=args.lr, betas=(0.9, 0.999))
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, [20,28], gamma=0.25, last_epoch=-1)
    ...
    dic='./saved_model/' + str(args.model) + '_rep_nofinetune' + str(args.sensing_rate) + '.pth'
    trainer.load(dic)
    accelerator.print('\n')
    ...
    accelerator.print('Finetuning... ')
    for epoch in range(args.epochs):
        accelerator.print('\ncurrent lr {:.5e}'.format(trainer.optimizer.param_groups[0]['lr']))
        loss = trainer.train_epoch(epoch,finetune=True)
        scheduler.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='krccsnet',
                        choices=['krccsnet'],
