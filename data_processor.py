...
def data_loader(args=None,bsds_size=480):
    
    if args is not None:
        args_batch_size=args.batch_size
    else:
        args_batch_size=1
    kwopt = {'num_workers': 2, 'pin_memory': True}
    ...
    test_bsds_transforms = torchvision.transforms.Compose([
        torchvision.transforms.CenterCrop((bsds_size,320 )),
        torchvision.transforms.ToTensor(),
    ])
    ...
    trn_loader = torch.utils.data.DataLoader(trn_dataset, batch_size=args_batch_size, shuffle=True, **kwopt,
                                             drop_last=False)
